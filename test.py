...
def group_merge(dev_group_id, user_group_id, new_df_s):
    ...
    #                         == dev_group_id)].index.tolist(), 'end_date'] = new_df_date
    # result_df.loc[result_df[(result_df.group_id == user_group_id) | (result_df.group_id \
    #                         == dev_group_id)].index.tolist(), 'exp_userid'] = exp_userid
    # result_df.loc[result_df[(result_df.group_id == user_group_id) | (result_df.group_id \
    #                         == dev_group_id)].index.tolist(), 'exp_dev'] = exp_dev
    # result_df.loc[result_df[(result_df.group_id == user_group_id) | (result_df.group_id \
    #                         == dev_group_id)].index.tolist(), 'next_group'] = new_group_id
    result_df.loc[result_df[(result_df.group_id == user_group_id) | (result_df.group_id \
        == dev_group_id)].index.tolist(), ['end_date', 'exp_userid', 'exp_dev', 'next_group']] = \
        [new_df_date, exp_userid, exp_dev, new_group_id]
    # 生成新记录
    # change_df['group_id'] = new_group_id
    # change_df['start_date'] = new_df_date
    # change_df['end_date'] = '2035-12-31'
    # change_df['eff_userid'] = exp_userid
    # change_df['eff_dev'] = exp_dev
...
def get_change_group(new_df_s, old_df):
    ...
    if not old_df[old_df.dev == new_df_dev].empty:
        dev_group_id = old_df[old_df.dev == new_df_dev].group_id.values[0]
    else:
        dev_group_id = None

    if not old_df[old_df.userid == new_df_user].empty:
        user_group_id = old_df[old_df.userid == new_df_user].group_id.values[0]
    else:
        user_group_id = None
...
def main():
    ...
    old_df.to_csv('test_group_old_re.csv', index=False, header=None, encoding="utf8", columns=result_columns)
    result_df.to_csv('test_group_result_re.csv', index=False, header=True, encoding="utf8", columns=result_columns)
