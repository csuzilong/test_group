...
def get_change_group(new_df_dev, new_df_user):
    """新记录带来的影响"""
    # global old_df
    is_dev_new = 1
    is_user_new = 1
...
def main():
    ...
    for id_t in new_df['seq_id']:
        ...
        # 获取用户或绑定关系已存在的群组
        (dev_group_id, user_group_id, is_dev_new, is_user_new) = get_change_group(new_df_dev, new_df_user)

        # 新dev
        if is_dev_new == 1:
            if is_user_new == 1 or pd.isnull(user_group_id):
                old_df = old_df.append(new_df_s, ignore_index=True)
            else:
                new_df_s.group_id = user_group_id
                old_df = old_df.append(new_df_s, ignore_index=True)
        # 老dev
            ...
            if pd.isnull(dev_group_id):
                # dev无群，生成新群
                create_new_group(new_df_s, new_df_dev, new_df_user)

            else:
                if pd.isnull(user_group_id):
                    pass
